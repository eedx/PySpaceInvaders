import pygame, random, math
from pygame import mixer


# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# CONVERTIR ARCHIVO EN EJECUTABLE (.EXE)
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

'''import io


def fuente_bytes(fuente):
    # Abre el archivo TTF u OTF en modo lectura binaria
    with open(fuente, 'rb') as f:
        # Lee todos los bytes del archivo y los almacena en una variable
        otf_bytes = f.read()
    # Crea un objeto BytesIO a partir de los bytes del archivo de la fuente
    return io.BytesIO(otf_bytes)


# Esta fuente debe pasarse luego a pygame.font.Font como Bytes
# fuente_como_bytes = fuente_bytes('nasalization-rg.otf')
# ESTA FUNCIÓN EJECUTARLA ANTES DE CADA LLAMADA A LA FUENTE
'''

# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# VARIABLES Y ELEMENTOS GRÁFICOS
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

# Inicializar Pygame
pygame.init()

# Crear la pantalla
pantalla = pygame.display.set_mode((800, 600))

# Titulo, Icono y fondo de pantalla
pygame.display.set_caption('Invasión Espacial')
icono = pygame.image.load('media/img/ovni.png')
pygame.display.set_icon(icono)
fondo = pygame.image.load('media/img/ai_space2.png')

# Agregar música
mixer.music.load('media/music/Away - Patrick Patrikios.mp3')
mixer.music.set_volume(0.3)
mixer.music.play(-1)    # -1 hace que se repita


# Variables del Jugador
img_jugador = pygame.image.load('media/img/astronave.png')
jugador_x = (800 - 64) / 2 
jugador_y = 585 - 64
jugador_x_cambio = 0

# Variables del Enemigo
img_enemigo = []
enemigo_x = []
enemigo_y = []
enemigo_x_cambio = []
enemigo_y_cambio = []
cantidad_enemigos = 8

for e in range(cantidad_enemigos):
    img_enemigo.append(pygame.image.load('media/img/enemigo.png'))
    enemigo_x.append(random.randint(0, 736))
    enemigo_y.append(random.randint(50, 200))
    enemigo_x_cambio.append(1)
    enemigo_y_cambio.append(50)

# Variables de la bala
balas = []
img_bala = pygame.image.load('media/img/bala.png')
bala_x = 0
bala_y = 585 - 64
bala_y_cambio = 2.5
bala_visible = False

# Variable puntaje
puntaje = 0
fuente = pygame.font.Font('media/font/nasalization-rg.otf', 32)
texto_x = 10
texto_y = 10

# Texto fin del juego
fuente_final = pygame.font.Font('media/font/nasalization-rg.otf', 44)


# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# FUNCIONES DEL JUEGO
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------


# Función jugador (nave espacial)
def jugador(x, y):
    pantalla.blit(img_jugador, (x, y))


# Función enemigo (ovni)
def enemigo(x, y, ene):
    pantalla.blit(img_enemigo[ene], (x, y))


# Función disparar bala
def disparar_bala(x, y):
    global bala_visible
    bala_visible = True
    pantalla.blit(img_bala, (x + 16, y + 10))


# Función calculo de colisiones
def hay_colision(x_1, y_1, x_2, y_2):
    distancia = math.sqrt((x_2 - x_1) ** 2 + (y_2 - y_1) ** 2)
    if distancia < 27:
        return True
    else:
        return False
    

# Función mostrar puntaje
def mostrar_puntaje(x, y):
    texto = fuente.render(f'Puntaje: {puntaje}', True, (255, 255, 255))
    pantalla.blit(texto, (x, y))


def texto_final():
    mi_fuente_final = fuente_final.render('FIN DEL JUEGO', True, (255, 255, 255))
    pantalla.blit(mi_fuente_final, (240, 250))


# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# LOOP PRINCIPAL DEL JUEGO
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

se_ejecuta = True
while se_ejecuta:
    
    # Imagen de fondo
    pantalla.blit(fondo, (0, 0))

    # Iterara eventos
    for evento in pygame.event.get():

        # Evento cerrar
        if evento.type == pygame.QUIT:
            se_ejecuta = False

        # Evento presionar teclas
        if evento.type == pygame.KEYDOWN:
            if evento.key == pygame.K_LEFT:
                jugador_x_cambio = -2
            if evento.key == pygame.K_RIGHT:
                jugador_x_cambio = 2
            if evento.key == pygame.K_SPACE:
                sonido_bala = mixer.Sound('media/music/Laser gun.mp3')
                sonido_bala.set_volume(0.6)
                sonido_bala.play()
                nueva_bala = {
                    'x': jugador_x,
                    'y': jugador_y,
                    'velocidad': -5
                }
                balas.append(nueva_bala)
    
        # Evento soltar flechas
        if evento.type == pygame.KEYUP:
            if evento.key == pygame.K_LEFT or evento.key == pygame.K_RIGHT:
                jugador_x_cambio = 0
                

    # Modificar posición del jugador
    jugador_x += jugador_x_cambio

    # Mantener dentro de límites al jugador
    if jugador_x <= 0:
        jugador_x = 0
    elif jugador_x >= 736:
        jugador_x = 736

    # Modificar posición del enemigo
    for e in range(cantidad_enemigos):

        # Comprobación de fin del juego
        if enemigo_y[e] > 470:
            balas.clear()
            for k in range(cantidad_enemigos):
                enemigo_y[k] = 1000
            texto_final()
            break

        enemigo_x[e] += enemigo_x_cambio[e]

        # Mantener dentro de límites al enemigo
        if enemigo_x[e] <= 0:
            enemigo_x_cambio[e] = 1
            enemigo_y[e] += enemigo_y_cambio[e]
        elif enemigo_x[e] >= 736:
            enemigo_x_cambio[e] = -1
            enemigo_y[e] += enemigo_y_cambio[e]

        # Verificación de colisión
        for bala in balas:
            colision_bala_enemigo = hay_colision(enemigo_x[e], enemigo_y[e], bala['x'], bala['y'])
            if colision_bala_enemigo:
                sonido_colision = mixer.Sound('media/music/Explosion.mp3')
                sonido_colision.play()
                balas.remove(bala)
                puntaje += 1
                enemigo_x[e] = random.randint(0, 736) 
                enemigo_y[e] = random.randint(50, 200)


        enemigo(enemigo_x[e], enemigo_y[e], e)

    # Movimiento de la bala
    for bala in balas:
        bala['y'] += bala['velocidad']
        pantalla.blit(img_bala, (bala['x'] + 16, bala['y'] + 10))
        if bala['y'] < 0:
            balas.remove(bala)


    jugador(jugador_x, jugador_y)

    mostrar_puntaje(texto_x, texto_y)
    

    # Actualizar pantalla
    pygame.display.update()
